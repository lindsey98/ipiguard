...
#   bash scripts/run_opi.sh ipiguard     # IPIGuard defense (forces plan-then-execute; see below)
#
...
# NOTE: ipiguard is a plan-then-execute defense; it overrides WORKFLOW_MODE=react to automatic at
# runtime (the react loop builds no committed plan to gate against).
