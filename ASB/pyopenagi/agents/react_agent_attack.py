    ...
    def run(self):
        ...
        if workflow:
            ...
            for i, step in enumerate(workflow):
                ...
                # IPIGuard: authorize each tool call against the committed plan; defer the rest.
                deferred_msgs = []
                if self.ipiguard_agent is not None and tool_calls:
                    authorized = []
                    for tool_call in self.normalize_tool_calls(tool_calls):
                        tname = tool_call.get("name")
                        ok, reason = self.ipiguard_agent.authorize(tname)
                        if ok:
                            authorized.append(tool_call)
                        else:
                            deferred_msgs.append(f"[IPIGuard] deferred tool '{tname}': {reason}")
                            self.logger.log(f"[IPIGuard] deferred tool '{tname}': {reason}\n", level="info")
                    tool_calls = authorized
                ...
                if tool_calls:
                    ...
                    # Every call this step was an off-plan action IPIGuard deferred: record and move on.
                    self.messages.append({
                        "role": "assistant",
                        "content": "[Observation]: " + ";".join(deferred_msgs),
                    })
