...
from agentdojo.agent_pipeline.basic_elements import InitQuery, SystemMessage, ReActQuery
...
from agentdojo.agent_pipeline.llms.react_llm import ReActOpenAILLM
...
def get_llm(provider: str, model: str, model_id: str | None, tool_delimiter: str) -> BasePipelineElement:
    if provider == "openai":
        ...
        # IPIGuard fork semantics: OpenAI-compatible server (e.g. vllm serve
        # --enable-auto-tool-choice) addressed by the model's served name.
        client = openai.OpenAI(
            api_key=os.getenv("LOCAL_API_KEY", "EMPTY"),
            base_url=os.getenv("LOCAL_BASE_URL", "http://localhost:8000/v1"),
        )
        llm = OpenAILLM(client, model)
    elif provider == "local_prompting":
        # upstream's LocalLLM: tool calls parsed from plain-text completions
...
class AgentPipeline(BasePipelineElement):
    ...
    def from_config(cls, config: PipelineConfig) -> Self:
        ...
        if config.defense == "reflection":
            ...
            client = llm.client
            llm = ReActOpenAILLM(client, config.llm)
            tools_loop = ReActToolsExecutionLoop([ReActToolsExecutor(), system_message_component, query_component, llm])
            pipeline = cls([system_message_component, query_component, llm, tools_loop])
            pipeline.name = llm_name
            return pipeline
