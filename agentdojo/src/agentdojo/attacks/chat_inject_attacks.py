...
# ChatInject's JSON in ./chatinject_data/ (see that directory's README for the source URLs).
#
# COVERAGE: ChatInject only generated this data for the banking / slack / travel injection GOALs. There is
# NO shopping/github/dailylife data -- an uncovered GOAL raises ValueError below. And the match is EXACT
# string equality on injection_task.GOAL, so if a suite reworded a GOAL vs the AgentDojo version ChatInject
# used, it misses even on a covered suite. Verify your suite's GOALs are keys in the JSON before trusting
# the numbers.
